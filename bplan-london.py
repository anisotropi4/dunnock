...
CODES = DATA['LOC'][['Location Code', 'Name']].set_index('Name')
LINKNAMES = set.union(set(DF1['Origin'].unique()), set(DF1['Destination'].unique()))


TERMINALS = pd.read_csv('London-Terminals.tsv', sep='\t')
TERMINALS = TERMINALS.set_index('Name').join(CODES).reset_index()

IDX1 = TERMINALS['Location Code'].isin(LINKNAMES)
TERMINALS = TERMINALS[IDX1].reset_index(drop=True)
...
LONDON = set(TERMINALS['Location Code'])
REGIONS = set(STATIONS) - LONDON

from itertools import product
EDGES = list(product(REGIONS, LONDON)) + list(product(LONDON, REGIONS))
...
TIMINGS.to_excel('BPLAN-London.xlsx', sheet_name='Link_Timing', index=False)
